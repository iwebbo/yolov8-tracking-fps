...
def custom_train(load='pre', traindata="valorant.yaml", epoch=50, batc=3, export=False, val=False):
    ...
    # Use the model
    model.train(data=traindata, epochs=epoch, batch=batc, device="cuda")
...
def custom_predict(sourc='screen', sav=True, sho=False,  imgs=(800,800), con=0.3, save_tx=False):
    ...
    predictions = model.predict(source=sourc, save=sav, show=sho, imgsz=imgs, conf=con, save_txt=save_tx, device="cuda")
...
if __name__ == '__main__':
    run_checks()
    custom_train(traindata="cs.yaml")
    #predict()
