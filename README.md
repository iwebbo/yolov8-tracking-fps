...
Ultralytics YOLOv8 (install via pip: pip install ultralytics)

onnx and onnxruntime (install via pip: pip install onnx onnxruntime)

A custom dataset annotated in YOLO format (using tools like LabelImg or Roboflow).

Training Steps with a Custom Dataset
...
Each .txt file must contain object annotations in YOLO format:

<class_id> <x_center> <y_center> <width> <height>

class_id: Class ID

x_center, y_center, width, height: Normalized coordinates (between 0 and 1).

Where to Find Custom Datasets?

👉 https://universe.roboflow.com

2. YAML Configuration File

Create a custom_dataset.yaml file containing:
...
Replace the paths and class names with those of your dataset.

3. Training Execution

Within this function, we define which model to use. Refer to the official documentation: Ultralytics GitHub

Features

Model Loading: Choose between a new model, pre-trained model, or transfer learning.

Training: Specify the number of epochs and batch size.

Validation: Option to validate the model after training.

Export: Ability to export the trained model in different formats.

Function Parameters:

load (str): Model loading method.

'new': Creates a new model from YAML.

'pre': Loads a pre-trained model.

'tran': Loads a model from YAML and applies transfer learning.

traindata (str): Path to training data configuration file (default: "fortnite.yaml").

epochs (int): Number of epochs (default: 50).

batch_size (int): Training batch size (default: 16).

export (bool): Whether to export the model after training (default: True).

val (bool): Whether to perform validation after training (default: True).

Train YOLOv8 using Ultralytics package:
...
The trained model will be saved in the runs/detect/custom_model/ directory.

Converting to ONNX Format

1. Conversion

After training, you can convert the YOLOv8 model to ONNX format for integration into your script:

# Export the model to ONNX format
model.export(format='onnx')

The ONNX file will be generated in the runs/detect/export/ directory.

2. Validating the PT Model
...
Place your trained YOLOv8 model (best_fort.pt) and test image (capture.jpg) in the same directory as the script.

Modify the script:

Update model_path and image_path if necessary.

Replace ['0', '1'] in class_names with actual class names.

Run the script:

python check_from_capture_pt_file_detect.py

✅ A window will display the image with detected objects, bounding boxes, class names, and confidence scores.

Integration into Detection Script

3. Validating the ONNX Model

Usage

Place your ONNX model (best_fort.onnx) and test image (capture.jpg) in the same directory as the script.

Modify the script:

Update model_path and image_path.
...
Place your trained YOLOv8 model (best_fort.pt) in the same directory as the script.
...
5. Run with an Arduino for Target Tracking

Hardware

An Arduino connected to your computer via the serial port.

A FPS game.

Usage

Place your YOLOv8 model (best_fort.pt) in the script directory.

Connect your Arduino and note the COM port (e.g., COM10).

Modify the script:

Replace 'COM10' with the correct port.
...
DEAD_ZONE: Dead zone around the screen center.

EXCLUDED_REGION: Screen exclusion zone (e.g., UI elements).
...
void loop() {
  if (Serial.available() > 0) {
    ...
    Mouse.move(x, y, 0); // Move the mouse (x, y, 0)
  }
}

Notes

Adjust the script to match your game’s settings.

Optimize SCALE_FACTOR, OFFSET_X, OFFSET_Y for better performance.

Conclusion

With these steps, you can train a YOLOv8 model on a custom dataset, convert it to ONNX, and integrate it into your projects.

🎯 Example FPS tracking: Streamable

📽️ YouTube Demo

If you want to train more models, visit 👉 https://universe.roboflow.com
